...
<style>
.activeStroke{
  filter: drop-shadow(3px 5px 2px rgba(1, 19, 155, 0.975));
}
</style>

<script>
export default {
  props: ['count', 'timerCount'],
  data () {
    return {
      maxSec: 0,
      size: '2.5rem',
      text: '',
      dashCount: 50, //max count
      activeCount: 20, // active count
      strokeWidth: 100,
      activeWidth: 100,
      stroke: '#B1144B',
      activeStroke: '#f40a0aad',
      dashSpacing: 0,
      rotate: -90,
      reverse: true,
    }
  ...
  watch: { 
    activeCount: function() { // watch it
      if (this.maxSec != this.activeCount) {
        this.stroke = '#f40a0aad'
      } else {
        this.stroke = '#162ae394'
      }
    },
  },
  methods: {
  ...
  created () {
    this.dashCount = this.timerCount;
    this.UNITS = 200;
    this.activeCount = this.count
    if (this.maxSec < this.activeCount) {
      this.maxSec = this.activeCount
    }
  },
}
</script>
