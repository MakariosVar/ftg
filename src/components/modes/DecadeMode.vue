...
export default {
    ...
    methods:{
        ...
        getData(){
            ...
            axios.get(API_URL+'/api/decade/'+this.$route.params.id).then( (response) => {
                if ( this.oldGames.length == 0 ||  !(this.oldGames.includes(response.data.selected))) {
                    ...
                    response.data.games.forEach((element, gameIndex) => {
                        element.companies.forEach( (company, companyIndex) => {
                            response.data.games[gameIndex][companyIndex] = company.name
                        })
                        if (element.id == this.selected) {
                            this.selectedGame = element
                        }
                    })
                    this.game1 = response.data.games[0]
                    this.game2 = response.data.games[1]
                    this.game3 = response.data.games[2]
                    this.game4 = response.data.games[3]
